...
[![Kotlin](https://cdn.videosdk.live/docs/images/youtube/Kotlin.png)](https://youtu.be/odjwcldqp74?si=VRp3otTobD-dbaeP "Kotlin")
...
- We have created 3 package screens and widgets in the following folder structure:
  - [`OneToOneCall`](app/src/main/java/live/videosdk/rtc/android/kotlin/OneToOneCall) - It includes all classes/files related to OneToOne meetings.
  - [`GroupCall`](app/src/main/java/live/videosdk/rtc/android/kotlin/GroupCall) - It includes all classes/files related to the Group meetings.
  - [`Common`](app/src/main/java/live/videosdk/rtc/android/kotlin/Common) - It includes all the classes/files that are used in both meeting type.

### [Common package](app/src/main/java/live/videosdk/rtc/android/kotlin/Common)

### 1. Pre-Call Setup on Join Screen

- [DeviceAdapter.kt](app/src/main/java/live/videosdk/rtc/android/kotlin/Common/Adapter/DeviceAdaptor.kt) : This is a custom `RecyclerView.Adapter` used to display a list of audio devices. It takes a list of devices and a click listener to handle item clicks. Each item shows the device name and an icon.
...
- [MessageAdapter.kt](app/src/main/java/live/videosdk/rtc/android/kotlin/Common/Adapter/MessageAdapter.kt) ; This is a custom `RecyclerView.Adapter` for displaying chat messages in a meeting.
...
- [`OneToOneCallActivity.kt`](app/src/main/java/live/videosdk/rtc/android/kotlin/OneToOneCall/OneToOneCallActivity.kt) : `OneToOneCallActivity.java` handles one-on-one video call, providing features like microphone and camera control, screen sharing, and participant management. It supports real-time chat and meeting event listeners for tasks like recording and screen sharing. The activity also displays session elapsed time and handles permissions for audio, video, and screen sharing.

### [GroupCall package](app/src/main/java/live/videosdk/rtc/android/kotlin/GroupCall)

- [`GroupCallActivity.kt`](app/src/main/java/live/videosdk/rtc/android/kotlin/GroupCall/Activity/GroupCallActivity.kt) : The `GroupCallActivity` class manages the main UI and logic for initiating and maintaining a group video call. It serves as the primary activity where users can join a video call session, toggle mic and camera. It also manages the video grid where all participants are displayed using `ParticipantViewFragment` and `ParticipantViewAdapter`.
- [`ParticipantViewFragment.kt`](app/src/main/java/live/videosdk/rtc/android/kotlin/GroupCall/Fragment/ParticipantViewFragment.kt) : Displays an individual participant's video feed and controls within a fragment, updating the UI based on participant state changes.
- [`ParticipantViewAdapter.kt`](app/src/main/java/live/videosdk/rtc/android/kotlin/GroupCall/Adapter/ParticipantViewAdapter.kt) : Binds participant data to a `RecyclerView`, dynamically updating the video grid as participants join, leave, or change state.
- [`ParticipantChangeListener.kt`](app/src/main/java/live/videosdk/rtc/android/kotlin/GroupCall/Listener/ParticipantChangeListener.kt) : Listens for participant-related events (join, leave, state changes) and triggers UI updates.
- [`ParticipantState.kt`](app/src/main/java/live/videosdk/rtc/android/kotlin/GroupCall/Utils/ParticipantState.kt) : Represents the current state of a participant, such as mute and video status, for UI display and logic handling.
